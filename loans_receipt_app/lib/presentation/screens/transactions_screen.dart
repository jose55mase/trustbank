...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  List<dynamic> get filteredTransactions {
    ...
    if (loanTypeFilter != null) {
      final beforeLoanTypeFilter = filtered.length;
      if (selectedFilter == 'Préstamo') {
    ...
    if (paymentFrequencyFilter != null) {
      final beforeFrequencyFilter = filtered.length;
      if (selectedFilter == 'Préstamo') {
        filtered = filtered.where((loan) {
    ...
    if (selectedDate != null) {
      ...
      filtered = filtered.where((item) {
        try {
          ...
          final matches = itemDate.year == selectedDate!.year &&
    ...
    if (selectedDateRange != null) {
      final beforeRangeFilter = filtered.length;
      filtered = filtered.where((item) {
        try {
          final dateField = selectedFilter == 'Préstamo' ? 'startDate' : 'date';
  ...
  Future<void> _exportTransactions(List<dynamic> transactions) async {
    try {
      ...
      for (int rowIndex = 0; rowIndex < transactions.length; rowIndex++) {
        ...
        if (isLoan) {
          // Para préstamos, usar el valor real de cuota guardado si existe
          if (item['valorRealCuota'] != null) {
            valorRealCuota = currencyFormat.format(item['valorRealCuota']);
          } else {
            // Fallback: calcular el valor real de cuota
          ...
          if (item['valorRealCuota'] != null) {
            ...
            if (loan != null) {
              // Usar el valor real de cuota guardado si existe
              if (loan['valorRealCuota'] != null) {
                valorRealCuota = currencyFormat.format(loan['valorRealCuota']);
              } else {
                // Fallback: calcular el valor
